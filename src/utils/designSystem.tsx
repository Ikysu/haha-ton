...
const icons = {
  ...
  SendTab: ['send', 'send-outline'],
  ListTab: ['list', 'list-outline'],
  ProfileTab: ['person-outline', 'person'],
};

export const getTabBarIcon =
  (tabName: keyof typeof icons) =>
  ({ focused, color, size }: { focused: boolean; color: string; size: number }) =>
    <Icon name={getTabIconName(tabName, focused)} size={size} color={color} />;

const getTabIconName = (tabName: keyof typeof icons, focused: boolean): string => {
  if (tabName in icons) {
    return icons[tabName][+focused];
  }
